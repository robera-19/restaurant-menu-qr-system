...
const formatItem = (item: any) => {
  const total = item.ratings.reduce(
    (sum: number, rating: any) => sum + rating.ratingValue,
    0,
  );
...
export const MenuService = {
  // get all menu
  getMenu: async (categoryId?: string, search?: string, adminView = false) => {
  ...
  // get menu by qr
  getMenuByQr: async (shortId: string) => {
  ...
  // create menu item
  create: async (data: any, adminId: string) => {
        ...
        data: {
          ...
          ...(body.price !== undefined &&
            toNum(body.price) !== null && {
              price: toNum(body.price) as number,
            }),

          ...(body.oldPrice !== undefined &&
            toNum(body.oldPrice) !== null && {
              oldPrice: toNum(body.oldPrice) as number,
            }),

          ...(body.preparationTime !== undefined &&
            toNum(body.preparationTime) !== null && {
              preparationTime: toNum(body.preparationTime) as number,
            }),

          ...(body.spicyLevel !== undefined &&
            toNum(body.spicyLevel) !== null && {
              spicyLevel: toNum(body.spicyLevel) as number,
            }),

          ...(body.calories !== undefined &&
            toNum(body.calories) !== null && {
              calories: toNum(body.calories) as number,
            }),
  ...
  // toggle availability
  toggle: async (id: string, isAvailable: boolean) => {
